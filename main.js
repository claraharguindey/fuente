...
const DESEO_PLACEHOLDER = "Un deseo para la fuente...";

const aText = ["Te acercas a una fuente", "#", "Haz scroll para mirar dentro"];
const bText = ["Miras en tu bolsillo", "#", "Encuentras una moneda"];
let delayTime = 0,
  delayAmount = 100;
...
function addCoin(data, nombre) {
  return getCoins().then(function (coins) {
    let id = "coin" + Date.now();
    coins[id] = { data: data, nombre: nombre };
...
function coinFontSize(text) {
  const len = text.length;
  if (len <= 60)  return "22pt";
  if (len <= 120) return "17pt";
  if (len <= 220) return "13pt";
  if (len <= 350) return "10pt";
  return "8pt";
}

function showCoin(data, nombre) {
  const fs = coinFontSize(data);
  const nameFs = nombre ? "calc(" + fs + " * 0.7)" : null;
  const nameHtml = nombre
    ? "<em style='font-size:" + nameFs + ";opacity:0.5'>" + nombre + "</em><br><br>"
    : "";
  const coinShow = document.getElementById("coin-show");
  coinShow.innerHTML =
    "<img class='moneda-grande' src='" + IMG_MONEDA + "'>" +
    "<p class='retrieve-text' style='font-size:" + fs + "'>" + nameHtml + data + "</p>";
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.getElementById("plus").addEventListener("click", function () {
    ...
    fade(document.getElementById("about"), "out", 200);
    document.getElementById("response").placeholder = DESEO_PLACEHOLDER;
    document.querySelector(".entry-page").innerHTML =
      "<img class='moneda-entrada' src='" + IMG_MONEDA + "'>";
    currentName = prompt("¿Cómo te llamas? (puedes dejarlo en blanco)") || "";
  ...
  // Submit
  document.getElementById("coin-input").addEventListener("submit", function (e) {
    e.preventDefault();
